...
test('Test post translator call', async (t) => {
    ...
    t.assert( result == '<p>Olá, mundo!</p>', 'it is translating correctly');
    t.end();
});

test('Test text translate', async (t) => {
    let text = 'Hello!';
    let sourceLang = 'en-US';
    let targetLang = 'fr';
    ...
    let result = await translator.translateText(translateParams);

    t.assert( result.length > 0, 'it is returning non empty result');
    t.assert( result == 'Bonjour!', 'it is translating correctly');
    t.end();
});

test('Test translator to long texts', async (t) => {
...
test('Test translator to long html texts', async (t) => {
...
test('Test bilingual translate', async (t) => {
    let text = `<p>Alice was beginning to get very tired of sitting by her sister on the bank, and of having nothing to do: once or twice she had peeped into the book her sister was reading, but it had no pictures or conversations in it, “and what is the use of a book,” thought Alice “without pictures or conversations?”</p>
    <br>
    <p>So she was considering in her own mind (as well as she could, for the hot day made her feel very sleepy and stupid), whether the pleasure of making a daisy-chain would be worth the trouble of getting up and picking the daisies, when suddenly a White Rabbit with pink eyes ran close by her.</p>
    <br />
